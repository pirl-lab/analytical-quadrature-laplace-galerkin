...
xlabel('1/h_4'); ylabel('Relative difference')
C=errL(neps)*ep1(neps);
lin=C./ep1;
loglog(ep1(1:3:length(ep1)),lin(1:3:length(ep1)),'k--');
B=errN(neps)*ep1(neps)*log(ep1(neps));
linlog=B./(ep1.*log(ep1));
loglog(ep1(1:3:length(ep1)),linlog(1:3:length(ep1)),'k-');
legend('Weakly singular','Hyper singular','err=C\epsilon','err=B\epsilon/log(\epsilon)')
